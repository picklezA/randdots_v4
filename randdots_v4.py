...
class Gen: # short for "Generator"
    ...
    def get_nearest_dots(dot_dict): # I think this can be multithreaded
        nearest = {}
        for key in dot_dict:
            sub_nearest = {}
    ...
    def draw_lines(new_image, random_dots_dict, nearest): # This is working great now
        for key in nearest:
    ...
    def line(point1, point2): # I think this can be optimized...Like with multithreading...
        all_points_between = []
        # y = mx + b
        # y - b / m = x
...
def run_new(num_wanted):    
    ...
    for i in range(1,num_wanted):
        ...
        time2 = time.time()
        delta_create_len = round(time2 - start_time, 8)
...
def on_run():
    ...
    while True:
        new_image_2, direction_dict, dot_array_next_step = move_image(blank_image.copy(), dot_array_next_step, direction_dict)
        img2 = ImageTk.PhotoImage(image=Image.fromarray((new_image_2.copy()).astype(numpy.uint8)))
        canvas.create_image(0, 0, anchor=tkinter.NW, image=img2)
        canvas.update()
    window.mainloop()
...
# if you want to pre-generate before display; run this one!
#run_new(10)

# if you want to generate on run; run this one!
#on_run()

# if you want to build a cache as you run; run this one!
dual.spawn_threads()